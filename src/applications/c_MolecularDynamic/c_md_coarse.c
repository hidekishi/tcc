/* ***********************************************************************
  ...
  DATE:              Nov 2024
  AUTHOR:            Bill Magro, Kuck and Associates, Inc. (KAI), 1998 (original)
  DESCRIPTION:       Coarse-grained version of molecular dynamics simulation.
                     Uses static scheduling with large chunk sizes to minimize overhead.
  GRANULARITY:       COARSE - Uses schedule(static, chunk) where chunk = np/threads.
                     Large contiguous blocks reduce synchronization overhead.
  COMMENTS:	         Modified from original to use static scheduling for coarse-grained parallelism.
  ...
  BASIC PRAGMAS:     parallel for with schedule(static, large_chunk)
	USAGE:             ./c_md_coarse.par 8192 10
...
/* -----------------------------------------------------------------------
   ...
   and velocities - COARSE-GRAINED VERSION
 * ----------------------------------------------------------------------- */
void compute(int np, int nd, vnd_t *pos, vnd_t *vel, 
  ...
  #pragma omp parallel
  {
    #pragma omp single
    {
      ...
      chunk_size = np / num_threads;
      if (chunk_size < 1) chunk_size = 1;
    }
  }
  
  /* The computation of forces and energies is fully parallel with coarse-grained scheduling */
#pragma omp parallel for default(shared) private(i, j, k, rij, d) reduction(+ : pot, kin) schedule(static, chunk_size)
  for (i = 0; i < np; i++) {
...
/* -----------------------------------------------------------------------
   Perform the time integration, using a velocity Verlet algorithm - COARSE-GRAINED VERSION
 * ----------------------------------------------------------------------- */
void update(int np, int nd, vnd_t *pos, vnd_t *vel, vnd_t *f, vnd_t *a, double mass, double dt) {
  int i, j;
  double rmass;
  ...
  /* The time integration is fully parallel with coarse-grained scheduling */
#pragma omp parallel for default(shared) private(i, j) firstprivate(rmass, dt) schedule(static, chunk_size)
  for (i = 0; i < np; i++) {
    for (j = 0; j < nd; j++) {
...
int main (int argc, char **argv) {
  ...
  OSCR_init (NUMTHREADS, "Molecular dynamic simulation (Coarse-Grained)", "Use md_coarse <Nparts> <Nsteps>", NUM_ARGS, 
