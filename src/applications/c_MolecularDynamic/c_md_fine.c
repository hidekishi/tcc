/* ***********************************************************************
  ...
  DESCRIPTION:       Fine-grained version of molecular dynamics simulation.
                     Uses dynamic scheduling with small chunk sizes for better load balancing.
  GRANULARITY:       FINE - Uses schedule(dynamic, 8) for particle-level load distribution.
                     Small chunks enable adaptive work distribution across threads.
  ...
  BASIC PRAGMAS:     parallel for with schedule(dynamic, 8)
	USAGE:             ./c_md_fine.par 8192 10
...
/* -----------------------------------------------------------------------
   ...
   and velocities - FINE-GRAINED VERSION
 * ----------------------------------------------------------------------- */
void compute(int np, int nd, vnd_t *pos, vnd_t *vel, 
  ...
  pot = 0.0;
  kin = 0.0;
  /* The computation of forces and energies is fully parallel with fine-grained scheduling */
#pragma omp parallel for default(shared) private(i, j, k, rij, d) reduction(+ : pot, kin) schedule(dynamic, 8)
  for (i = 0; i < np; i++) {
...
/* -----------------------------------------------------------------------
   Perform the time integration, using a velocity Verlet algorithm - FINE-GRAINED VERSION
 * ----------------------------------------------------------------------- */
void update(int np, int nd, vnd_t *pos, vnd_t *vel, vnd_t *f, vnd_t *a, double mass, double dt) {
  int i, j;
  double rmass;
  ...
  rmass = 1.0/mass;
  /* The time integration is fully parallel with fine-grained scheduling */
#pragma omp parallel for default(shared) private(i, j) firstprivate(rmass, dt) schedule(dynamic, 8)
  for (i = 0; i < np; i++) {
    for (j = 0; j < nd; j++) {
...
int main (int argc, char **argv) {
  ...
  OSCR_init (NUMTHREADS, "Molecular dynamic simulation (Fine-Grained)", "Use md_fine <Nparts> <Nsteps>", NUM_ARGS, 
