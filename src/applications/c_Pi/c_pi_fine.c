/* ***********************************************************************
  ...
  Boston, MA  02111-1307  USA
	...
	FILE:              c_pi_fine.c
  VERSION:           1.0 (Fine-Grained)
  DATE:              November 2024
  COMMENTS TO:       sande@csi.ull.es
  DESCRIPTION:       Parallel implementation of PI generator using OpenMP - Fine Grained Version
  COMMENTS:          Fine-grained version with dynamic scheduling and smaller chunk sizes
                     Uses schedule(dynamic, 1) for maximum load balancing
                     Better for irregular workloads and many threads
...
int main(int argc, char *argv[]) {
  ...
  int NUMTHREADS;
	...
	OSCR_init (NUMTHREADS, "Pi generator (Fine-Grained)", "Param: precission", NUM_ARGS, PARAM_NAMES, DEFAULTS_VALUE , NUM_TIMERS, NUM_TIMERS, TIMERS_NAMES, argc, argv); 
	N = OSCR_getarg_int(NUM_ARGS);
	OSCR_timer_start(0);
  w = 1.0 / N;
  pi = 0.0;
  
  /* Fine-grained parallelism: dynamic scheduling with chunk size 1 for maximum load balancing */
#pragma omp parallel for default(shared) private(i, local) reduction(+:pi) schedule(dynamic, 1)
  for(i = 0; i < N; i++) {
    local = (i + 0.5) * w;
