/* ***********************************************************************
  ...
  Boston, MA  02111-1307  USA
	...
	FILE:              c_mandel_fine.c
  VERSION:           1.0 (Fine-Grained)
  DATE:              November 2024
  AUTHOR:            F. de Sande
  COMMENTS TO:       sande@csi.ull.es
  DESCRIPTION:       This program computes an estimation to the 
	                   Mandelbrot Set area using MonteCarlo sampling - Fine Grained Version
										 The best known estimate so far is 1.50659177 +- 0.00000008.
  COMMENTS:	         Fine-grained version with dynamic scheduling for better load balancing
                     Uses schedule(dynamic, 10) to handle irregular iteration times
                     Each point may require different iteration counts
...
int main(int argc, char **argv) {
  ...
  char *DEFAULT_VALUES[NUM_ARGS] = {"4092"};
   ...
   OSCR_init (NUMTHREADS, "Mandelbrot set area (Fine-Grained)", "Use 'mandel' <Number of points>", NUM_ARGS,
 ...
 *    Fine-grained: Use dynamic scheduling with small chunks for load balancing
 */
	OSCR_timer_start(0);
  outside = 0;
#pragma omp parallel for default(none) reduction(+:outside)       \
	                       private(i, j, ztemp, z) shared(NPOINTS, points) schedule(dynamic, 10)
  for(i = 0; i < NPOINTS; i++) {
    z.re = points[i].re;
    z.im = points[i].im;
    for (j = 0; j < MAXITER; j++) {
