/* ***********************************************************************
  ...
  Boston, MA  02111-1307  USA
	...
	FILE:              c_mandel_coarse.c
  VERSION:           1.0 (Coarse-Grained)
  DATE:              November 2024
  AUTHOR:            F. de Sande
  COMMENTS TO:       sande@csi.ull.es
  DESCRIPTION:       This program computes an estimation to the 
	                   Mandelbrot Set area using MonteCarlo sampling - Coarse Grained Version
										 The best known estimate so far is 1.50659177 +- 0.00000008.
  COMMENTS:	         Coarse-grained version with static scheduling and large chunks
                     Uses schedule(static, NPOINTS/NUMTHREADS) for reduced overhead
                     Better when iteration times are more uniform
  ...
  BASIC PRAGMAS:     parallel for
	USAGE:             ./c_mandel_coarse.par 8192
...
int main(int argc, char **argv) {
  ...
  char *DEFAULT_VALUES[NUM_ARGS] = {"4092"};
   ...
   OSCR_init (NUMTHREADS, "Mandelbrot set area (Coarse-Grained)", "Use 'mandel' <Number of points>", NUM_ARGS,
  ...
  /* Calculate large chunk size for coarse-grained parallelism */
  chunk_size = NPOINTS / NUMTHREADS;
 ...
 *    Coarse-grained: Use static scheduling with large chunks for reduced overhead
 */
	OSCR_timer_start(0);
  outside = 0;
#pragma omp parallel for default(none) reduction(+:outside)       \
	                       private(i, j, ztemp, z) shared(NPOINTS, points, chunk_size) schedule(static, chunk_size)
  for(i = 0; i < NPOINTS; i++) {
    z.re = points[i].re;
    z.im = points[i].im;
    for (j = 0; j < MAXITER; j++) {
